...
class ImportLoan(models.TransientModel):
    _name = "import.loan"
    ...
    def get_check_employee_loan(self, employee_id):
        ...
        loan_ids = self.env['employee.loan'].search(
            [('employee_id', '=', employee_id.id), ('date', '<=', e_date), ('date', '>=', s_date)])
        return len(loan_ids)
        ...
        for line in lines:
            ...
            if not employee_id:
                ...
                if emp_loan >= employee_id.loan_request:
                    remark = self.add_in_remark(remark, 'Employee Can not create more then ' + str(
                        employee_id.loan_request)) + ' Loans'
